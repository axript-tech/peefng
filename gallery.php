...
<body class="bg-brand-light-bg">
        ...
        <section class="py-20">
            <div class="container mx-auto px-6">
                ...
                <div id="filter-buttons" class="flex justify-center flex-wrap gap-4 mb-12">
                    ...
                    <!-- Album filters will be loaded here by jQuery -->
                </div>

                <!-- Gallery Grid -->
                <div class="grid grid-cols-8 md:grid-cols-8 lg:grid-cols-8 gap-10" id="gallery-grid">
                    <!-- Gallery items will be loaded here by jQuery -->
                </div>
            </div>
        </section>
    </main>
    ...
    <script>
        $(document).ready(function(){
            ...
            // AJAX to fetch albums and create filters
            $.ajax({
                ...
                success: function(response) {
                    if(response.status === 'success' && response.data) {
                        ...
                        response.data.forEach(album => {
                            const filterButton = `<button class="filter-button bg-gray-200 text-gray-800 font-semibold py-2 px-6 rounded-full" data-filter=".${album.slug}">${album.name}</button>`;
            ...
            // AJAX to fetch gallery images
            $.ajax({
                url: 'php/api/gallery.php',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    if(response.data) {
                        ...
                        response.data.forEach(image => {
                            const galleryItem = `
                                <div class="gallery-item ${image.album_slug}">
                                    <a href="${image.image_path}" data-fancybox="gallery" data-caption="${image.title}">
                                        <img src="${image.image_path}" alt="${image.title}" class="w-full h-full object-cover">
                                        <div class="gallery-overlay">
                                            <h4 class="text-white font-bold">${image.title}</h4>
                                        </div>
                                    </a>
                                </div>`;

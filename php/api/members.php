...
// update, and delete member data from the database.
...
if ($method === 'GET') {
    ...
    if ($_POST['action'] === 'create') {
        ...
        $email = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL);
        $phone = sanitize_input($_POST['phone_number']);
        $tierId = filter_input(INPUT_POST, 'membership_tier_id', FILTER_SANITIZE_NUMBER_INT);
        $isActive = filter_input(INPUT_POST, 'is_active', FILTER_SANITIZE_NUMBER_INT);

        if (empty($memberId) || empty($fullName) || empty($email) || empty($tierId)) {
            $response['message'] = 'Please fill in all required fields.';
        } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $response['message'] = 'Invalid email format.';
        } else {
            try {
                $sql = "UPDATE users SET full_name = ?, email = ?, phone_number = ?, membership_tier_id = ?, is_active = ? WHERE id = ? AND role = 'Member'";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$fullName, $email, $phone, $tierId, $isActive, $memberId]);
                
                $response['status'] = 'success';
                $response['message'] = 'Member details updated successfully!';
            } catch (PDOException $e) {
                http_response_code(500);
                $response['message'] = 'Database error: ' . $e->getMessage();
            }

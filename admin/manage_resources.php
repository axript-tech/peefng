...
<head>
    ...
    <title>Manage Resources - Admin Dashboard - PEEF</title>
    ...
    <div class="d-flex">
        ...
        <aside class="sidebar text-white d-none d-lg-flex flex-column p-3">
            ...
            <nav class="nav flex-column my-4">
                ...
                <a href="manage_resources.php" class="sidebar-link active d-flex align-items-center p-3"><i class="fas fa-book text-center" style="width: 24px;"></i><span class="ms-3">Resources</span></a>
        ...
        <div class="main-content d-flex flex-column">
            ...
            <header class="bg-white shadow-sm p-4 d-flex justify-content-between align-items-center">
                ...
                <h1 class="h2 font-bold text-brand-dark mb-0 d-none d-lg-block">Manage Resources</h1>
                    ...
                    <div class="card shadow-sm border-0">
                        <div class="card-body">
                            <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center mb-4">
                                <h2 class="card-title h5 font-bold text-brand-dark mb-3 mb-sm-0">Resource Library</h2>
                                <button id="add-resource-btn" class="btn btn-success" style="background-color: var(--brand-green); border-color: var(--brand-green);">
                            ...
                            <div class="table-responsive">
                                <table id="resources-table" class="table" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th>File Name</th>
                                            <th>Type</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="save-resource-btn" class="btn btn-primary" style="background-color: var(--brand-green); border-color: var(--brand-green);">Save Resource</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="delete-confirm-modal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">Are you sure you want to delete this resource: <strong id="resource-name-to-delete"></strong>?</div>
                <div class="modal-footer">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#resources-table').DataTable({
                "ajax": { "url": "../php/api/resources.php", "dataSrc": "data" },
                "columns": [
                    { "data": "title" },
                    { "data": "file_type", "render": function(data) {
                        let icon = 'fa-file';
                        if (data.includes('pdf')) icon = 'fa-file-pdf text-danger';
                        else if (data.includes('doc')) icon = 'fa-file-word text-primary';
                        else if (data.includes('xls')) icon = 'fa-file-excel text-success';
                    ...
                    { 
                        ...
                        "render": function(data, type, row) {
                            return `<a href="../${row.file_path}" target="_blank" class="btn btn-light btn-sm action-btn" title="Download"><i class="fas fa-download text-success"></i></a>
                                    <button class="btn btn-light btn-sm action-btn edit-btn" title="Edit" data-id="${data}"><i class="fas fa-edit text-primary"></i></button>
            ...
            $('#add-resource-btn').on('click', function() {
                ...
                $('#resourceModalLabel').text('Add New Resource');
                $('#action').val('create');
                $('#resource_id').val('');
            ...
            $('#save-resource-btn').on('click', function() {
                const formData = new FormData($('#resource-form')[0]);
                $.ajax({
                    type: 'POST', url: '../php/api/resources.php', data: formData, dataType: 'json', contentType: false, processData: false,
            ...
            $('#resources-table tbody').on('click', '.edit-btn', function() {
                const rowData = table.row($(this).parents('tr')).data();
                $('#resourceModalLabel').text('Edit Resource');
                $('#action').val('update');
                $('#resource_id').val(rowData.id);
            ...
            $('#resources-table tbody').on('click', '.delete-btn', function() {
                resourceIdToDelete = $(this).data('id');
                $('#resource-name-to-delete').text($(this).data('name'));
                deleteModal.show();
            });
            ...
            $('#confirm-delete-btn').on('click', function() {
                if (resourceIdToDelete) {
                    $.ajax({
                        type: 'POST', url: '../php/api/resources.php', data: { action: 'delete', id: resourceIdToDelete }, dataType: 'json',
                        success: function(response) {
                            if (response.status === 'success') {
                                showToast(response.message, 'success');
                                table.ajax.reload();
                        ...
                        complete: function() { deleteModal.hide(); resourceIdToDelete = null; }
                    });
                }
            });
